...
use xmpp_parsers::{pubsub, BareJid, Jid};
...
impl PubSubQuery {
    ...
    pub fn set_to(mut self, to: JID) -> Self {
        self.to = Some(to);
        self
    }
    ...
    pub fn build(self) -> Iq {
        ...
        if let Some(to) = self.to {
            iq.to = Some(Jid::Bare(
                BareJid::new(to.to_string().as_str()).unwrap_or_else(|e| {
                    panic!("Could not convert `entity::model::JID` to `xmpp_parsers::Jid`: {e}")
                }),
            ));
        }
        iq
